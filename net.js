...
@compute @workgroup_size(1) fn main(@builtin(workgroup_id) gindex: vec3<u32>,@builtin(local_invocation_id) lindex: vec3<u32>) {
  ...
  data0[alu0] = select(acc0,(acc0+3549),(acc0<0));
  data0[alu1] = select(acc1,(acc1+3549),(acc1<0));
  data0[alu2] = select(acc2,(acc2+3549),(acc2<0));
}`;

const r_1183_2_3549_3_3 = `fn nan() -> f32 { let bits = 0xffffffffu; return bitcast<f32>(bits); }
@group(0) @binding(0)
var<uniform> INFINITY : f32;
@group(0) @binding(1)var<storage,read_write>data0:array<f32>;
@group(0) @binding(2)var<storage,read_write>data1:array<f32>;
...
@compute @workgroup_size(2) fn main(@builtin(workgroup_id) gindex: vec3<u32>,@builtin(local_invocation_id) lindex: vec3<u32>) {
  ...
  var acc7 = 0.0f;
  var acc8 = 0.0f;
  for (var ridx2 = 0; ridx2 < 3549; ridx2++) {
    var val3 = data3[ridx2];
    var alu2 = (alu0+(ridx2*6));
    var val4 = data1[alu2];
    var val5 = data1[(alu2+1)];
    var val6 = data1[(alu2+2)];
    var cast0 = (f32(((val0!=val3)!=true)));
    var cast1 = (f32(((val1!=val3)!=true)));
    var cast2 = (f32(((val2!=val3)!=true)));
    acc0 = (acc0+(val4*cast0));
    acc1 = (acc1+(val4*cast1));
    acc2 = (acc2+(val4*cast2));
    acc3 = (acc3+(val5*cast0));
    acc4 = (acc4+(val5*cast1));
    acc5 = (acc5+(val5*cast2));
    acc6 = (acc6+(val6*cast0));
    acc7 = (acc7+(val6*cast1));
    acc8 = (acc8+(val6*cast2));
  }
  var alu13 = ((gidx0*18)+alu0);
  data0[alu13] = acc0;
  data0[(alu13+1)] = acc3;
  data0[(alu13+2)] = acc6;
  data0[(alu13+6)] = acc1;
